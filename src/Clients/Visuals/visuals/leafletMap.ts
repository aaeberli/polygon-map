    ...
    export class LeafletMap implements IVisual {
        ...
        public static converter(dataView: DataView): GeoDatapoint[] {
            ...
            function parseLineString(wkt) {
                ...
                var removeNonNumeric = replaceWhite.replace(/[^0-9.,-]/g, "");
                var pushToArray = removeNonNumeric.split(",");
                var reversed = pushToArray;
                //console.log('reversed WKT: ' + reversed);
                reversed = reversed.filter(function (n) { return n !== ""; });
                //console.log('cleaned WKT: ' + reversed);
                for (var i = 0, polyLength = reversed.length / 2; i < polyLength; i++) {
                    var lng = reversed[i * 2];
                    var lat = reversed[i * 2 + 1];

                    var floatLat = parseFloat(lat + "");
                    var floatLng = parseFloat(lng + "");
                    var LatLngObj = null;
                    try {
                        LatLngObj = new L.LatLng(floatLat, floatLng);
                    }
                    catch (ex) {
                        LatLngObj = null;//new L.LatLng(51.169931, -4.669326);
                    }
            ...
            var columnsCount = dataView.table.columns.length;
            var dataPoints: GeoDatapoint[] = [];

            for (var i = 0, rowLength = data.length; i < rowLength; i++) {
                ...
                switch (wktTypeCheck) {
                    ...
                    case "POL":
                        ...
                        for (var c = 0; c < columnsCount; c++) {
                            if (data[i] != null && data[i][c] != undefined)
                                mergedRow += data[i][c] + "";
                        }
                        if (mergedRow.substr(mergedRow.length - 2, 2) == "))") {
                            var polygonData = parseLineString(/*data[i][0]*/mergedRow);
                            dataPoints.push({
                                geoType: "POLYGON",
                                geoData: polygonData,
                                description: "test",
