    ...
    !function(data) {
        ...
        !function(DataViewPivotCategoricalToPrimaryGroups) {
            ...
            function inferUnpivotTransform(selects, dataViewMappings, dataView, projectionActiveItems) {
                ...
                var roleKinds = data.DataViewSelectTransform.createRoleKindFromMetadata(selects, dataView.metadata), projections = data.DataViewSelectTransform.projectionsFromSelects(selects, projectionActiveItems);
                ...
                for (var currentGroup, groups = [], i = 0, len = values.length; len > i; i++) {
                    ...
                    if (!currentGroup || currentGroup.identity !== value.identity) {
                        if (currentGroup = {
                            ...
                            source.objects && (currentGroup.objects = source.objects);
                        }
        ...
        !function(utils) {
            ...
            !function(DataViewMatrixUtils) {
                ...
                function visitLeafNodesRecursive(matrixNode, nextIndex, callback) {
                    if (_.isEmpty(matrixNode.children)) callback(matrixNode, nextIndex), nextIndex++; else for (var children = matrixNode.children, i = 0, len = children.length; len > i; i++) {
                        var nextChild = children[i];
                        nextChild && (nextIndex = visitLeafNodesRecursive(nextChild, nextIndex, callback));
                    }
                    return nextIndex;
                }
